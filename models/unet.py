...
class UNet3D_hved(nn.Module):
    ...
    def forward(self, x):
        ...
        for l in range(4):
            encoder_out.append(self.encoders[l](x[:,l:l+1]))   #  [x1,x2,x3,x4]
...
class UNet3D(nn.Module):
    ...
    def forward(self, x):
        c1 = self.conv1a(x)     # torch.Size([1, 32, 80, 80, 80])
        c1 = self.conv1b(c1)    # torch.Size([1, 32, 80, 80, 80])
        c1d = self.ds1(c1)      # torch.Size([1, 64, 40, 40, 40])
        #print("c1d shape:", c1d.shape)
        
        c2 = self.conv2a(c1d)
        c2 = self.conv2b(c2)
        c2d = self.ds2(c2)      # torch.Size([1, 128, 20, 20, 20])
        c2d_p = self.pool(c2d)  # torch.Size([1, 128, 10, 10, 10])
#         print("c2d shape:", c2d_p.shape)
        
        c3 = self.conv3a(c2d)
        c3 = self.conv3b(c3)
        c3d = self.ds3(c3)      # torch.Size([1, 256, 10, 10, 10])
#         print("c3d shape:", c3d.shape)

        c4 = self.conv4a(c3d)
        c4 = self.conv4b(c4)
        c4 = self.conv4c(c4)
        c4d = self.conv4d(c4)   # torch.Size([1, 256, 10, 10, 10])
#         print("c4d shape:", c4d.shape)
...
if __name__=="__main__":
    net = UNet3D_hved(input_shape=[80,80,80], in_channels=1)
    
    in_tensor = torch.randn(1,4,80,80,80)
